...
from src.models import OPTConfig
...
from src.dptrainer_aggzo import Trainer  # pick the variant with --zo_method dpzo|sc|lc
...
class DynamicTrainingArguments(TrainingArguments):
    ...
    estimate_clip: bool = field(
        default=False,
        metadata={
            "help": 'whether to estimate clipping threshold'}
    )
    dpzero_clip_threshold: float = field(
        default=1.0,
        metadata={"help": "DPZero clip threshold"}
    )
    dpzero_base_lr: float = field(
        default=1e-4,
        metadata={"help": "Base learning rate numerator; effective lr = dpzero_base_lr / dpzero_clip_threshold"}
    )
    zo_method: str = field(
        default="dpzo",
        metadata={"help": "AggZO gradient post-processing: 'dpzo' (plain), 'sc' (symmetric extension), 'lc' (localization correction)"}
    )
    sc_expand: int = field(
        default=2,
        metadata={"help": "zo_method=sc only: symmetric-extension factor, 1 (off), 2 ([g, -g.flip]) or 8"}
    )
    sc_compensate: bool = field(
        default=True,
        metadata={"help": "zo_method=sc only: multiply the kept n directions by sqrt(2) after clipping"}
    )
    ...
    dp_sample_rate: float = field(
        default=0.9,
        metadata={"help": "DP sampling rate for batch selection"}
    )
    # DP-AggZO args
    n: int = field(
        default=1,
        metadata={'help': 'number of random z\'s per record.'}
    )
    random_direction_seed: int = field(
        default=-1,
        metadata={"help": "Base seed for random directions and DP noise (Gaussian and Laplace) in DPZero. When set, ensures reproducible but different random values across iterations. When -1, uses truly random seeds."}
    )
    # Debugging
    sampler_seed: int = field(
        default=None,
        metadata={"help": "Seed for the UniformWithReplacementSampler to ensure reproducible batch sampling across runs. When not set, falls back to data_seed or args.seed."}
    )
...
def main():
    ...
    # local_model_path = "./model/roberta-base"
    # local_model_path = "./model/distilroberta-base"
    ...
    # local_model_path = "./model/facebook/opt-1.3b"

    # # 1) AutoConfig: detect roberta vs opt from the local model directory
    ...
    # # 2) few-shot type: pick the matching roberta / opt class out of MODEL_TYPES
    # if 'prompt' in model_args.few_shot_type:
    #     model_fn = MODEL_TYPES[config.model_type]
    # elif model_args.few_shot_type == 'finetune':
    #     if training_args.from_linearhead:
    ...
    # # 3) tokenizer: AutoTokenizer likewise matches roberta / opt automatically
    # tokenizer = AutoTokenizer.from_pretrained(
    #     local_model_path,
    #     additional_special_tokens=special_tokens,
    #     cache_dir=model_args.cache_dir,
    ...
    # # 4) load the model (both modes unchanged)
    # if training_args.hf_inference_model:
    #     free_in_GB = int(torch.cuda.mem_get_info()[0]/1024**3)
    #     max_memory = f'{free_in_GB-5}GB'
    #     n_gpus = torch.cuda.device_count()
    ...
    # # 5) tie embeddings: roberta and opt both support tie_emb, skip other types
    # if training_args.tie_emb:
    #     if model.config.model_type in ["roberta", "opt"]:
    #         logger.warn(f"Tie embeddings for {model.config.model_type}")
    #         model.tie_emb()
    #     else:
    #         logger.warn(f"tie_emb=True but model_type={model.config.model_type} does not implement tie_emb, skipping.")

    # # 6) head_tuning: for both roberta and opt the head is called lm_head
    # if training_args.head_tuning:
    #     if model.config.model_type in ["roberta", "opt"]:
    #         head_name = "lm_head"
    #     else:
    #         raise ValueError(f"head_tuning is not defined for model_type={model.config.model_type}")

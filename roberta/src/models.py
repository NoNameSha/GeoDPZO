...
from transformers import OPTModel, OPTForCausalLM, PreTrainedModel
from transformers import OPTConfig as HFOPTConfig
from transformers.models.auto.configuration_auto import CONFIG_MAPPING


class OPTConfig(HFOPTConfig):
    r"""
    自定义 OPT 配置，兼容 dpzero / LoRA / num_k 等参数。

    使用方式与 transformers.OPTConfig 完全一致，只是多了几个自定义字段：
      - dpzero: 是否启用 DPZERO 相关逻辑
      - num_k: 你的算法里用到的 k
      - apply_lora, lora_r, lora_alpha: LoRA 相关参数
    """
    model_type = "opt-1.3b"   # 原生 OPT 就是 "opt"，这里显式写出来保持一致
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # 对于 OPT，pad/bos/eos 一般已经在预训练权重的 config.json 里设好了，
        # 如果需要你也可以在 kwargs 里覆盖，这里不强行改默认值
...
class OPTModelForPromptFinetuning(PreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        ...
        # lm head (OPTForCausalLM exposes it)
        self.lm_head = OPTForCausalLM(config).lm_head
    ...
    def tie_emb(self):
        """OPTIONAL: OPT also allows weight tying like LLaMA"""
        input_embeddings = self.opt.get_input_embeddings()
        output_embeddings = self.lm_head

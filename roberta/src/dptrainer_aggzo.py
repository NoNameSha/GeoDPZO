...
dpzo  <- src/dptrainer_aggzo_est.py           plain DP-AggZO (no post-processing)
sc    <- src/dptrainer_aggzo_symmetry.py      symmetric extension before clipping
lc    <- src/dptrainer_aggzo_localization.py  sorted localization correction
...
class Trainer(LinearHeadTrainer):
    ...
    def __init__(self, model=None, args=None, data_collator=None, train_dataset=None,
        ...
        method = getattr(self.args, "zo_method", None) or os.environ.get("ZO_METHOD", "dpzo")
        method = str(method).strip().lower()
        if method not in ZO_METHODS:
            raise ValueError(
                f"unknown zo_method {method!r}, expected one of {ZO_METHODS}"
            )
        self.zo_method = method
        logger.info(f"[DP-AggZO] gradient post-processing method: {self.zo_method}")

    def _norm_path(self, epoch=0):
        """Path of the per-sample gradient-norm dump used for clip estimation.

        The three original trainers hard-coded different ``k`` tags (k1 / k16 / k256);
        here it follows ``args.n``, override with the ``NORM_K_TAG`` env var to read
        an older dump.
        """
        k_tag = os.environ.get("NORM_K_TAG", f"k{self.args.n}")
        return (
            "./src/norm_copy/all_norms_" + self.task_name.upper() + "_roberta-large"
            + f"_{k_tag}_eps6-epoch{epoch}.npy"
        )
    ...
    def get_full_dataloader(self) -> DataLoader:
        """
        Return a full-sample dataloader (no replacement, one full pass over the dataset).
        """
        if self.train_dataset is None:
            raise ValueError("Trainer: training requires a train_dataset.")
        ...
        # DistributedSampler when running distributed, SequentialSampler otherwise
        if self.args.world_size <= 1:
            sampler = SequentialSampler(train_dataset)   # sequential, no replacement
        else:
            sampler = DistributedSampler(train_dataset)  # shard the data across GPUs

        return DataLoader(
            train_dataset,
            sampler=sampler,
            batch_size=1,   # one sample at a time, so per-sample grads are easy to get
            collate_fn=data_collator,
            num_workers=self.args.dataloader_num_workers,
            pin_memory=self.args.dataloader_pin_memory,
            worker_init_fn=seed_worker,
    ...
    # ------------------------------------------------------------------
    # method-specific post-processing of the [n, batch] projected gradients
    # ------------------------------------------------------------------

    def symmetry_cdf_gap(self, grads):
        """
        grads: [K, B]

        return:
            scalar symmetry gap (smaller is better)

        Metric:
            mean_{i,b} |g_(i,b) + g_(K-i+1,b)|
        """

        # ===== Step 1: sort =====
        sorted_vals, _ = torch.sort(grads, dim=0)   # [K, B]

        K = sorted_vals.shape[0]
        half = K // 2

        # ===== Step 2: pair matching =====
        left = sorted_vals[:half]                # [half, B]
        right = sorted_vals.flip(0)[:half]       # [half, B]

        # ===== Step 3: symmetry gap =====
        pair_gap = torch.abs(left + right)       # [half, B]

        return pair_gap.mean()

    def _symmetry_extend(self, stacked_grads):
        """`sc`: mirror the K directions with their negatives before clipping."""
        return torch.cat([stacked_grads, -stacked_grads.flip(0)], dim=0)   # [2n, batch]

    def _localization_correct(self, stacked_grads):
        """`lc`: sort each column, cancel the mean of the k* outermost pairs, unsort."""
        # ===== Step 1: sort =====
        # sort each sample independently (column-wise)
        sorted_vals, sorted_indices = torch.sort(stacked_grads, dim=0)

        g_bar = sorted_vals.clone()   # shape: [K, batch]

        K = g_bar.shape[0]

        # ===== Step 2: find k* =====
        # compute pair sums
        pair_sum = torch.abs(g_bar + g_bar.flip(0))   # [K, batch]

        # compute differences
        diff = torch.abs(pair_sum[1:] - pair_sum[:-1])   # [K-1, batch]

        # k* per sample
        k_star = torch.argmax(diff, dim=0)   # shape: [batch]
        # print("k_star", k_star)

        # ===== Step 3: localization correction =====
        g_bar_corrected = g_bar.clone()

        for b in range(g_bar.shape[1]):   # iterate over batch
            ks = k_star[b].item()

            for i in range(ks + 1):   # python index starts from 0
                j = K - i - 1

                mean_pair = (g_bar[i, b] + g_bar[j, b]) / 2

                g_bar_corrected[i, b] = g_bar[i, b] - mean_pair
                g_bar_corrected[j, b] = g_bar[j, b] - mean_pair

        # ===== Step 4: recover original order =====
        # inverse permutation
        g_recovered = torch.zeros_like(g_bar_corrected)
        g_recovered.scatter_(0, sorted_indices, g_bar_corrected)

        return g_recovered

    def _apply_zo_method(self, stacked_grads):
        """Dispatch on ``zo_method``; returns the matrix that gets clipped."""
        if self.zo_method == "dpzo":
            return stacked_grads

        if self.zo_method == "sc":
            return self._symmetry_extend(stacked_grads)

        # lc: also track how symmetric the gradients are before/after the correction
        pair_before = self.symmetry_cdf_gap(stacked_grads)
        g_recovered = self._localization_correct(stacked_grads)
        pair_after = self.symmetry_cdf_gap(g_recovered)

        step_id = self.state.global_step
        if step_id < self.pair_record.shape[0]:
            self.pair_record[step_id, 0] = pair_before
            self.pair_record[step_id, 1] = pair_after
        print("pair_before", pair_before)
        print("pair_after", pair_after)
    ...
    def train(self, model_path=None, dev_objective=None):
        ...
        if self.args.dpzero:
            if self.args.dp_epsilon > 0:
                multiplier = get_noise_multiplier(
                    target_epsilon=self.args.dp_epsilon * 1 ,  
                ...
                print(f'expected batch size {noisy_batch_size}')
        ...
        if self.zo_method == "lc" and not hasattr(self, "pair_record"):
            T_steps = self.args.max_steps
            self.pair_record = torch.zeros(T_steps, 2, device=self.args.device)
        ...
        for epoch in range(epochs_trained, int(num_train_epochs)):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if self.args.zero_order_optim:
                    ...
                    #     save_dir = os.path.join("./norm_bricks", "stacked_grads")
                    #     os.makedirs(save_dir, exist_ok=True)
                    #     model_name = "roberta-base"            
                    #     save_path = os.path.join(
                    #         save_dir,
                    #         f"stacked_grads_{self.task_name.lower()}_{model_name}_k{self.args.n}_{self.state.global_step}.pt"
                    #     )

                    #     torch.save(stacked_grads.detach().cpu(), save_path)
                    #     print(f"[SAVE] {save_path}")

                    # method-specific post-processing: dpzo / sc / lc
                    processed_grads = self._apply_zo_method(stacked_grads)

                    # clip
                    if self.args.dp_epsilon > 0:
                        norms = torch.norm(processed_grads, dim=0, keepdim=True)
                        ...
                        # norms = torch.norm(processed_grads, dim=0, keepdim=True)
                        # gamma = 1e-5
                        # scaling_factors = self.args.dpzero_clip_threshold / (norms + gamma)
                    else:
                        scaling_factors = 1
                    clipped_grads = processed_grads * scaling_factors

                    if self.zo_method == "sc":
                        # keep the first n directions of the mirrored matrix
                        clipped_grads = clipped_grads[: self.args.n]
                        if getattr(self.args, "sc_compensate", True):
                            clipped_grads = clipped_grads * math.sqrt(2)
                    ...
                    for j in range(self.args.n):
                        ...
                        self.args.learning_rate = self.args.dpzero_base_lr / self.args.dpzero_clip_threshold
                    ...
                    if self.zo_method == "lc":
                        pair_cpu = self.pair_record.detach().cpu().numpy()
                        np.save(f"PAIR_record_SORTED_{self.task_name}.npy", pair_cpu)
                    ...
                    if self.state.global_step == 1:
                        _eta_step2_start = time.time()
                    elif self.state.global_step == 2:
                        step_sec = time.time() - _eta_step2_start
                        remaining_sec = step_sec * (t_total - 2)
                        logger.info(
                            f"[ETA] step2 took {step_sec:.1f}s → remaining: "
                            f"{remaining_sec/3600:.1f}h ({remaining_sec/60:.0f}min) "
                            f"for {t_total - 2} steps left"
                        )
            ...
            # if self.args.estimate_clip and epoch in [0, num_train_epochs // 2]:
            #     mse_path_1 = "mse1_result" + self.task_name.upper() + "eps" + str(self.args.dp_epsilon) + str(epoch) + ".npy"
            #     np.save(mse_path_1, np.array(mse1_list))

            #     mse_path_2 = "mse2_result" + self.task_name.upper() + "eps" + str(self.args.dp_epsilon) + str(epoch) + ".npy"
            #     np.save(mse_path_2, np.array(mse2_list))

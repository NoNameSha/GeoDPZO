...
class MrpcProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        """See base class."""
        logger.info("LOOKING AT {}".format(os.path.join(data_dir, "train_sampled_1024.tsv")))
        return self._create_examples(self._read_tsv(os.path.join(data_dir, "train_sampled_1024.tsv")), "train")
...
class GoEmotionsProcessor(DataProcessor):
    """
    Data processor for text classification datasets go_emotions.
    """
    ...
    def get_train_examples(self, data_dir):
        """See base class."""
        return self._create_examples(pd.read_csv(os.path.join(data_dir, "train_sampled_1024.csv"), header=None).values.tolist(), "train")
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            elif self.task_name in ['mr', 'sst-5', 'subj', 'trec', 'cr', 'mpqa', 'go_emotions']:
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            if self.task_name == "ag_news":
                ...
                examples.append(InputExample(guid=guid, text_a=line[1], short_text=line[1], label=line[0]))
            elif self.task_name == "yahoo_answers_topics":
                text = line[1]
                if not pd.isna(line[2]):
                    text += ' ' + line[2]
                if not pd.isna(line[3]):
                    text += ' ' + line[3]
                examples.append(InputExample(guid=guid, text_a=text, short_text=line[1], label=line[0]))
            elif self.task_name in ['mr', 'sst-5', 'subj', 'trec', 'cr', 'mpqa', 'yahoo_answers']:
                examples.append(InputExample(guid=guid, text_a=line[1], label=line[0]))
            else:
                raise Exception("Task_name not supported.")
...
processors_mapping = {
    ...
    "go_emotions": GoEmotionsProcessor("go_emotions"),
...
compute_metrics_mapping = {
    ...
    "go_emotions": text_classification_metrics,

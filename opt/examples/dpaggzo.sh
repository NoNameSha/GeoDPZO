# HF_ENDPOINT=https://hf- irror.com CUDA_VISIBLE_DEVICES=0 MODEL=facebook/opt-1.3b TASK=SST2 MODE=ft LR=5e-6 EPS=1e-3 DP_SAMPLE_RATE=0.064 DP_EPS=2.0 STEPS=1000 N=64 DP_CLIP=25 bash examples/dpaggzo.sh
#
# To pick the AggZO variant (all three now live in src/dptrainer_aggzo.py):
#   ZO_METHOD=dpzo|sc|lc TASK=SST2 bash examples/dpaggzo.sh
...
echo "ZO_METHOD: $ZO_METHOD"
echo "DP_BASE_LR: $DP_BASE_LR (effective lr = $DP_BASE_LR / $DP_CLIP)"
...
python run_dpaggzo.py \
    ...
    --evaluation_strategy no --save_strategy no --save_total_limit 1 \
    --eval_steps $EVAL_STEPS  \
    --train_as_classification \
    --dpzero \
    --zo_method $ZO_METHOD \
    --dpzero_clip_threshold $DP_CLIP \
    --dpzero_base_lr $DP_BASE_LR \
...
echo "DP_EPS: $DP_EPS"
echo "DP_CLIP: $DP_CLIP"
echo "Estimate_clip: $ESTIMATE_CLIP"
echo "ZO_METHOD: $ZO_METHOD"
echo "number of directions: $N"
# echo "Extra args: $EXTRA_ARGS $TASK_ARGS"

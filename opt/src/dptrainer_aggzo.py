...
Unified DP-AggZO trainer. Merges the three previously separate trainers into one
file, selected at runtime with ``--zo_method`` (or the ``ZO_METHOD`` env var):

    dpzo  <- src/dp_trainer_aggzo.py              plain DP-AggZO (no post-processing)
...
Everything outside :meth:`OurTrainer._apply_zo_method` is shared by all three.
"""
...
# from src.estimate_clip import DPNormPartitionedMax
# from src.svt_estimate_clip import Quantile_estimate
...
class OurTrainer(Trainer):
    ...
    def _symmetry_extend(self, stacked_grads):
        """`sc`: mirror the n directions with their negatives before clipping."""
        return torch.cat([stacked_grads, -stacked_grads], dim=0)   # [2n, batch]
    ...
    def _localization_correct(self, stacked_grads):
        ...
        g_bar = sorted_vals.clone()
        K = g_bar.shape[0]

        # ===== Step 2: find k* =====
        pair_sum = torch.abs(g_bar + g_bar.flip(0))
        diff = torch.abs(pair_sum[1:] - pair_sum[:-1])
        k_star = torch.argmax(diff, dim=0)

        # ===== Step 3: localization correction =====
        g_bar_corrected = g_bar.clone()
        for b in range(g_bar.shape[1]):
            ks = k_star[b].item()
            for i in range(ks + 1):
                j = K - i - 1
    ...
    def _apply_zo_method(self, stacked_grads):
        ...
        return self._localization_correct(stacked_grads)
    ...
    def _inner_training_loop(
        ...
        if self.args.dpzero:
            if self.args.dp_epsilon > 0:
                multiplier = get_noise_multiplier(target_epsilon=self.args.dp_epsilon* 1, #0.95
                                                target_delta=self.args.dp_delta,
                                                steps=max_steps,
                                                sample_rate=self.args.dp_sample_rate,
                                                )
                #multiplier = 9.39453125

                print("noise multiplier", multiplier)                                
                # perturb size of dataset
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                for multi_d_idx in range(self.args.n):
                    ...
                    # forward
                    scaling_factor = 1
                    scalar = scaling_factor * self.args.zo_eps  
                ...
                all_elemnts = torch.cat([g.view(-1) for g in grads])
                std = torch.std(all_elemnts)
                # Stack 
                ...
                #         save_dir,
                #         f"stacked_grads_{self.args.task_name.lower()}_{model_name}_k{self.args.n}_{self.state.global_step}.pt"
                #     )
                ...
                if self.args.dp_epsilon > 0:
                    ...
                    # sc divides by the norm of the mirrored matrix, guard against 0
                    denom = norms + 1e-12 if self.zo_method == "sc" else norms
                    scaling_factors = self.args.dpzero_clip_threshold / denom
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0 or (
                    ...
                    for j in range(self.args.n):
                        ...
                        for name, param in self.named_parameters_to_optim:
                            ...
                            learning_rate_coupled = self.args.dpzero_base_lr / self.args.dpzero_clip_threshold
                            z = torch.normal(mean=0,std=1,size=param.data.size(),device=param.data.device,dtype=param.data.dtype)                            
                            if "bias" not in name and "layer_norm" not in name and "layernorm" not in name:
                                param.data = param.data - learning_rate_coupled * (noisy_mean_grad[j] * z + args.weight_decay * param.data)
                            else:
                                param.data = param.data - learning_rate_coupled * (noisy_mean_grad[j] * z)
                            del z
    ...
    def dpzero_clip(self, loss_diff, C=1.):
        ...
        num_less_than_one = torch.sum(tmp<1).item()
        return torch.mul(tmp, loss_diff)


    def dpzero_step(self, model, inputs, noisy_batch_size):
        """
        ...
        if self.args.dp_epsilon > 0:
            # poisson divided by noisy expected batch size
            self.projected_grad = self.dpzero_clip(loss_diff, self.args.dpzero_clip_threshold).sum() / noisy_batch_size

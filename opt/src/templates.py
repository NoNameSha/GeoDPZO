...
class SST5Template(Template):
    # 0..4（按你数据集 label 定义来；这里用最常见的 5 档情感）
    verbalizer = {
        0: "terrible",
        1: "bad",
        2: "okay",
...
class SNLITemplate(Template):
    ...
    def _get_pair(self, sample):
        premise = (sample.data.get("premise") or sample.data.get("sentence1") or "").strip()
        hypothesis = (sample.data.get("hypothesis") or sample.data.get("sentence2") or "").strip()
        return premise, hypothesis

    def encode(self, sample):
        premise, hypothesis = self._get_pair(sample)
        return (
            f"Premise: {premise}\n"
            f"Hypothesis: {hypothesis}\n"
            f"Therefore, the hypothesis is"
        )
    ...
    def verbalize(self, sample, candidate):
        ...
        return (
            f"Premise: {premise}\n"
            f"Hypothesis: {hypothesis}\n"
            f"Therefore, the hypothesis is {self.verbalizer[candidate]}"
        )
    ...
    def verbalize_sfc(self, sample, candidate):
        return f"\nTherefore, the hypothesis is {self.verbalizer[candidate]}"


class MNLITemplate(Template):
    # 0 entailment / 1 neutral / 2 contradiction（MNLI 和 SNLI 一样的三分类）
    verbalizer = {
        0: "entailed",
        1: "unknown",
        2: "contradicted",

...
from src.dptrainer_aggzo import OurTrainer  # pick the variant with --zo_method dpzo|sc|lc
...
class OurArguments(TrainingArguments):
    ...
    # Model loading
    model_name: str = "facebook/opt-1.3B" # HuggingFace model name
    ...
    dpzero_base_lr: float = 8e-4 # effective lr = dpzero_base_lr / dpzero_clip_threshold
    zo_method: str = "dpzo" # AggZO post-processing: 'dpzo' (plain), 'sc' (symmetric extension), 'lc' (localization correction)
    sc_compensate: bool = True # zo_method=sc only: multiply the kept n directions by sqrt(2) after clipping
    estimate_clip: bool = False 
    dp_epsilon: float = 6.0
    dp_delta: float = 1e-5
    
    # Poisson sample
    dp_sample_rate: float = 0.008
...
class Framework:
    ...
    def forward(self, input_ids, option_len=None, generation=False):
        ...
        This function is only for inferencefloat16
        """
    ...
    if args.train_set_seed is not None or args.num_train_sets is not None:
        ...
        for train_set_id, train_samples in enumerate(train_sets):
            ...
            if args.trainer != "none":
                ...
                # Zero-shot evaluation on the test set before training
                if not args.no_eval:
                    zero_shot_metrics = framework.evaluate([], eval_samples)
                    logger.info("===== Zero-shot (test) =====")
                ...
                if not args.no_eval:
                    ...
                    for m in zero_shot_metrics:
                        metrics["zero_shot_" + m] = zero_shot_metrics[m]
